...
class FitnessMeasureServiceImpl(
    ...
    @Transactional(readOnly = true)
    override fun getFitnessMeasure(
        memberId: Long,
        measureDay: LocalDate?,
    ): FitnessMeasureResponse {
        // 1) 해당 날짜의 내 체력 측정 기록 조회
        val measure: FitnessMeasure = if (measureDay != null) {
            fitnessMeasureRepository
                .findByMemberIdAndMeasureDayAndDeletedYn(memberId, measureDay, "N")
                ?: throw GlobalException(ErrorStatus.FITNESS_NOT_FOUND)
        } else {
            // 요청 날짜 없으면 가장 최근 기록
            fitnessMeasureRepository
                .findTopByMemberIdAndDeletedYnOrderByMeasureDayDesc(memberId, "N")
                ?: throw GlobalException(ErrorStatus.FITNESS_NOT_FOUND)
        }
        ...
        val compMeasures = latestPerMember
            .filter { it.memberId != measure.memberId }

        // 5) totalRank 계산
        val totalRank = calculateTotalRank(measure, compMeasures)

        // 6) 응답 DTO 매핑
        return FitnessMeasureResponse.from(measure, totalRank)
    }
